...
def display_word_char(base_word: str, guessed_word: list, guess_: str):
    ...
    if guess_ not in base_word:
        ...
        while guess_ in base_word:  # 'fills in' the letter on its spot(s) on the underscores and removes it from the word being checked in the while loop (so it terminates)
...
def display_choices(letters_: str, guess_: str):
    ...
    if guess_ == word or len(guess) > 1:  # handles if the user guesses the entire word correctly or gives any other multi-char response
...
if __name__ == '__main__':
    ...
    with open('words.txt') as words:
        wordlist = [word for word in words.read().split()]  # creates a list of words to pull a word from using the text file 'words.txt'

    word = wordlist[rr(len(wordlist))].lower()  # randomly generates an index based on the amt of words in the txt file and uses it to select a word
