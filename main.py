...
def display_word_char(base_word: list, guessed_word: list, guess_: str):
...
def display_word_chars(base_word: list, guessed_word: list, guess_: str):
    """Changes the visuals of the guessed word (underscores), deducts lives, and handles guessing for
        all inputs of multiple characters"""
    global lives  # func can now edit the global var lives
    base_word = ''.join(base_word)
    if guess_ == base_word:
        # checks for if the user gets the whole word right first try
        print('Remaining Lives:', lives)
        return guess_
    else:
...
def display_choices(letters_: str, guessword: str):
    letters_ = list(letters_)
    for letter in guessword:
        if letter.upper() in letters_:
            letters_[letters_.index(letter.upper())] = '_'
    return ''.join(letters_)


if __name__ == '__main__':
    print('=' * 100)
    print('Welcome to Hangman! Or Guess The Word, given that there is no hanging man.'
          '\nYou determine how many lives you get, which represent the amount of times you'
          '\ncan mess up before you lose.')
    ...
    with open('words.txt') as words:
        wordlist = [word for word in words.read().split()]
    ...
    while not lives.isdigit() or int(lives) < 1:
        lives = input('Please provide a positive integer number of lives.\nHow many lives would you like? ')
    lives = int(lives)
    ...
    while '_' in guess_word:
        if lives > 0:
            ...
            if len(guess) > 1:
                guess_word = list(display_word_chars(_word, guess_word, guess))
            else:
                guess_word = list(display_word_char(_word, guess_word, guess))
            letters = display_choices(letters, guess_word)
